...
const MakeADecision = () => {
  ...
  const handleDecisionClick = (decision) => {
    ...
    navigate(`${ROUTES.SCORECARD}?isblank=false&id=675ae4528811b601343a9457`); // Update this route as needed
    setIsModalVisible(false);
  };
          ...
          {decision.map((v, k) => (
            <div className="border-gray-400 border-b-2" key={k}>
              <div className="flex flex-row items-center gap-7 pb-6 mt-10">
                <img src={v.image} className="w-20 h-20" alt="life" />
                <div>
            ...
            {selectedDecisionsLocal.map((decision, index) => (
              <li
                ...
                className={`flex items-center gap-2 text-primary ${
                  decision.hasAccess ? "cursor-pointer hover:underline" : ""
                } `}
                onClick={
                  decision.hasAccess
