...
import { CHANGE_IN_SCORECARDS } from "../../utilities/localStorageKeys";
...
const MultiDynamicEditableTable = ({
    ...
    return columns.some((v) => {
      if (v?.dataIndex?.startsWith("weighting")) {
        ...
        const allItemsHaveValue = dataSource.every(
          (s) => s[v.dataIndex] !== undefined && s[v.dataIndex] !== null
        );
  ...
  const addColumn = (key) => {
    if (key === "add") {
      setObjectInLocalStorage(CHANGE_IN_SCORECARDS, {
        ...prevScorecardsNewChanges,
        [scorecardId]: "yes",
      });
    }
  ...
  const handleClickOnSave = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        message.success("Decision is being saved");
        setDecisionInfo(FORM_DATA);
        handleCancel();
        //  remove current scorecardId
        const { scorecardId, ...rest } = prevScorecardsNewChanges;
        setObjectInLocalStorage(CHANGE_IN_SCORECARDS, rest);
        setisSaveNewScorecardMdel(false);
        return;
      }
      message.error("We are facing some issue in facing decision");
    } catch (error) {
      message.error("We are facing some issue in facing decision");
    } finally {
  ...
  useEffect(() => {
    if (
      prevScorecardsNewChanges &&
      prevScorecardsNewChanges[scorecardId] === "yes"
    ) {
